...
async function masterLogin(){
  ...
  if(e!==MASTER_ADMIN_EMAIL){ alert('Master Admin email is incorrect.'); return; }
  if(!p){ alert('Password is required.'); return; }
  try{
    if(!window.firebase || !firebase.auth){ throw new Error('Firebase Authentication is not loaded'); }
    const cred=await firebase.auth().signInWithEmailAndPassword(MASTER_ADMIN_EMAIL,p);
    if(!cred.user || cred.user.email.toLowerCase()!==MASTER_ADMIN_EMAIL) throw new Error('Unauthorized account');
    masterLoggedIn=true; sessionRole='master';
    document.getElementById('masterLoginBox').style.display='none';
    document.getElementById('masterPanel').style.display='block';
    status.innerText='✅ Master Admin logged in — Firebase Authentication verified';
    document.getElementById('competitorView').classList.add('hidden');
    document.getElementById('adminView').classList.remove('hidden');
    addMasterControlButton();
    try{renderQuestions();}catch(e){} try{loadPastEvents();}catch(e){} try{loadResults();}catch(e){}
    loadOrganizers();
  }catch(err){ console.error(err); alert('Master Admin login failed. Check the Firebase Authentication email/password.'); }
}
