...
async function masterLogin(){
  ...
  if(e!==MASTER_ADMIN_EMAIL){alert('Master Admin login failed. Use the registered Master Gmail.');return;}
  if(!p){alert('Enter the Firebase Authentication password.');return;}
  try{
    if(!firebase.auth) throw new Error('Firebase Authentication SDK not loaded.');
    const cred=await firebase.auth().signInWithEmailAndPassword(e,p);
    masterAuthUser=cred.user;
    if((masterAuthUser.email||'').toLowerCase()!==MASTER_ADMIN_EMAIL){await firebase.auth().signOut();throw new Error('This account is not the Master Admin account.');}
    masterLoggedIn=true; sessionRole='master';
    localStorage.setItem('noor_master_logged_in','1');
    ...
    try{renderQuestions();}catch(e){} try{loadPastEvents();}catch(e){} try{loadResults();}catch(e){}
    loadOrganizers();
  }catch(err){console.error('Master login error:',err);alert('Master Admin login failed. Check the Firebase Authentication password for '+MASTER_ADMIN_EMAIL+'.');}
}
