...
try {
  ...
  setTimeout(() => {
    ...
    if (s) s.innerText = '✅ Firebase 8.10.1 + REST + Local - Secure - All Checked - Connected - V9.17.2 FINAL SECURE FULL - NABIDINAM SPECIAL';
    const si = document.getElementById('storageInfo');
    if (si) si.innerText = '✅ Secure - All Checked - 3 Layer - Event Board + Banner Logo + Login Secure + All Options Fixed';
  }, 800);
} catch (e) {
  const s = document.getElementById('firebaseStatus');
  if (s) s.innerText = '⚠️ SDK Failed REST works: ' + e.message;
}
...
function init() {
  ...
  if (document.getElementById('certX')) {
    document.getElementById('certX').value = sp.x;
    document.getElementById('certY').value = sp.y;
    document.getElementById('certS').value = sp.s;
    updateCertLive();
  }
...
async function activateOrganizer() {
  const email = prompt('Organizer email:', '');
  if (email === null) return;
  const code = prompt('Activation code:', '');
  if (code === null) return;
  const e = email.trim().toLowerCase(), c = code.trim();
  if (!e || !c) { alert('Email and activation code are required.'); return; }
  try {
    const data = await masterDb('masterAdmin/organizers');
    const match = Object.entries(data || {}).find(([id, o]) => String(o.email || '').trim().toLowerCase() === e && String(o.activationCode || '') === c && o.active !== false);
    if (!match) { alert('Invalid activation code/email or this organizer is disabled.'); return; }
    const [id, o] = match;
    const phone = prompt('Set your Admin phone number:', '');
    if (phone === null) return;
    const pass = prompt('Set your Admin password (minimum 6 characters):', '');
    if (pass === null) return;
    if (!phone.trim() || pass.trim().length < 6) { alert('Phone number is required and password must be at least 6 characters.'); return; }
    localStorage.setItem('noor_organizer_id', id);
    localStorage.setItem('noor_admin_phone', phone.trim());
    localStorage.setItem('noor_admin_pass', pass.trim());
    ...
    alert('✅ Organizer activated. You can now login with your new Admin phone and password.');
  } catch (err) { alert('Activation failed. Please check Firebase connection.'); }
}
...
function renderQuestions() { const l = document.getElementById('qList'), t = document.getElementById('totalQCount'), s = document.getElementById('showQCount'); if (t) t.innerText = questions.length; if (s) s.innerText = Math.min(eventQuestionCount || questions.length, questions.length); if (!questions.length) { l.innerHTML = '<p class="text-center py-6 text-gray-500">No Questions - Bulk Add ചെയ്യൂ!</p>'; document.getElementById('quota').innerText = 'Quota: 0'; return; } l.innerHTML = ''; const d = Math.min(eventQuestionCount || questions.length, questions.length); questions.slice(0, d).forEach((q, i) => { l.innerHTML += `<div class="bg-white border-2 rounded-xl p-3 mb-2 flex justify-between gap-2 shadow-sm"><div class="flex-1"><p class="font-bold text-[13px]">${i + 1}. ${q.q} ${randomMode ? '🔀 Random' : ''}</p><p class="text-[11px] text-gray-600 mt-1">${q.opts.map((o, idx) => idx === q.ans ? `✅ ${o}` : o).join(' | ')}</p></div><button onclick="deleteQuestion(${i})" class="bg-red-100 text-red-600 border-2 px-4 py-1 rounded-full text-[11px] font-black">🗑️ Delete</button></div>` }); document.getElementById('quota').innerText = 'Total: ' + questions.length + ' | Show: ' + d + ' | Random:' + (randomMode ? 'ON' : 'OFF') }
function bulkAdd() { const txt = document.getElementById('qBulk').value.trim(); if (!txt) { alert('Type Questions!'); return; } let a = 0; txt.split('\n').forEach(l => { if (!l.trim()) return; const p = l.split('|'); if (p.length < 2) return; const q = p[0].trim(), o = p[1].split(',').map(x => x.trim()).filter(x => x); let ans = 0; if (p[2]) { ans = parseInt(p[2].trim()); if (isNaN(ans)) ans = 0 } if (q && o.length >= 2) { questions.push({ q, opts: o, ans: Math.min(ans, o.length - 1) }); a++ } }); localStorage.setItem('questions_v13', JSON.stringify(questions)); document.getElementById('qBulk').value = ''; renderQuestions(); alert('✅ ' + a + ' Added!'); }
function deleteQuestion(i) { if (!confirm('Delete Q ' + (i + 1) + '?')) return; questions.splice(i, 1); localStorage.setItem('questions_v13', JSON.stringify(questions)); renderQuestions(); }

async function loadPastEvents() {
  const c = document.getElementById('pastEventsList');
  const role = getUserRole(), ownerId = getOrganizerId();
  if (!c) return;
  c.innerHTML = '<p class="text-xs text-center py-6">☁️ Loading TRUE CLOUD...</p>';
  try {
    ...
    if (res.ok) {
      ...
      if (data && Object.keys(data).length > 0) {
        ...
        Object.keys(data).reverse().filter(id => role === 'master' || (data[id] && data[id].ownerId === ownerId)).forEach(id => {
          const ev = data[id];
          c.innerHTML += `<div class="bg-white border-[3px] rounded-xl p-4 mb-3 shadow-md"><div class="flex justify-between gap-3"><div class="flex-1"><p class="font-black text-[14px] text-green-800">${ev.title || 'Noor Event'} <span class="bg-green-100 px-2 py-1 rounded-full text-[10px]">${ev.minute || 10} Min</span> <span class="bg-blue-100 px-2 py-1 rounded-full text-[10px]">Count:${ev.count || 20}</span> <span class="bg-orange-100 px-2 py-1 rounded-full text-[10px]">Random:${ev.randomMode ? 'ON' : 'OFF'}</span></p><p class="text-[12px] font-black text-gray-800 mt-2">📍 ${ev.place || 'No Place'} | ${ev.category || ''} | Qs:${ev.questions?.length || 0}</p><p class="text-[11px] text-gray-600 mt-1">Logo:${ev.logo ? '✅' : '❌'} Banner:${ev.banner ? '✅' : '❌'} Cert:${ev.certificate ? '✅' : '❌'} | ID:${id}</p></div><div class="flex flex-col gap-2"><button onclick="copyEventLink('${id}')" class="bg-green-600 text-white px-5 py-2.5 rounded-full text-[11px] font-black shadow">📋 Copy Link</button><button onclick="editEventById('${id}')" class="bg-blue-700 text-white px-5 py-2.5 rounded-full text-[11px] font-black shadow">✏️ Edit</button><button onclick="deleteEvent('${id}')" class="bg-red-100 text-red-600 border-2 px-5 py-2.5 rounded-full text-[11px] font-black">🗑️ Delete</button></div></div></div>`;
        });
        document.getElementById('storageInfo').innerText = '✅ REST API SUCCESS - ' + Object.keys(data).length + ' Events Loaded';
        localStorage.setItem('pastEvents_backup', JSON.stringify(data));
        return;
      } else {
        c.innerHTML = '<p class="text-xs text-center py-6 text-green-600 font-black">✅ REST Connected! No Events Yet!</p>';
        document.getElementById('storageInfo').innerText = '✅ REST Connected - No Events Yet';
        return;
      }
    }
  } catch (e) { }
  try {
    if (firebaseDB) {
      ...
      if (data) {
        ...
        Object.keys(data).reverse().filter(id => role === 'master' || (data[id] && data[id].ownerId === ownerId)).forEach(id => {
          const ev = data[id];
          c.innerHTML += `<div class="bg-white border-2 rounded-xl p-3 mb-2"><div class="flex justify-between"><div class="flex-1"><p class="font-black text-xs">${ev.title || 'Event'} - ${ev.place || ''}</p><p class="text-[10px]">ID:${id} | Logo:${ev.logo ? '✅' : '❌'} Banner:${ev.banner ? '✅' : '❌'}</p></div><div class="flex flex-col gap-1"><button onclick="copyEventLink('${id}')" class="bg-green-600 text-white px-4 py-1.5 rounded-full text-[11px] font-black">📋 Copy</button><button onclick="editEventById('${id}')" class="bg-blue-600 text-white px-4 py-1.5 rounded-full text-[11px] font-black">✏️ Edit</button></div></div></div>`;
        });
        document.getElementById('storageInfo').innerText = '✅ SDK Loaded - ' + Object.keys(data).length + ' Events';
        localStorage.setItem('pastEvents_backup', JSON.stringify(data));
        return;
      }
    }
  ...
  try {
    ...
    if (localData && Object.keys(localData).length > 0) {
      c.innerHTML = '<p class="text-[11px] text-center py-2 bg-yellow-100 rounded-full border">⚠️ Showing Local Backup</p>';
      Object.keys(localData).reverse().filter(id => role === 'master' || (localData[id] && localData[id].ownerId === ownerId)).forEach(id => {
        ...
        c.innerHTML += `<div class="bg-yellow-50 border-2 rounded-xl p-3 mb-2"><div class="flex justify-between"><div class="flex-1"><p class="font-black text-xs">${ev.title || 'Event'} - Local Backup</p><p class="text-[10px]">${ev.place || ''} | ID:${id}</p></div><div class="flex flex-col gap-1"><button onclick="copyEventLink('${id}')" class="bg-green-600 text-white px-3 py-1 rounded-full text-[10px] font-bold">📋 Copy</button><button onclick="editEventById('${id}')" class="bg-blue-600 text-white px-3 py-1 rounded-full text-[10px] font-bold">✏️ Edit</button></div></div></div>`;
      });
      document.getElementById('storageInfo').innerText = '⚠️ Local Backup Shown';
      return;
    }
  } catch { }
  c.innerHTML = `<p class="text-xs text-center py-6 text-red-600 font-bold">❌ Connection Failed!</p>`;
}

function clearAllEvents() { if (!confirm('Delete ALL?')) return; fetch(`${REST_URL}/events.json`, { method: 'DELETE' }).then(() => { localStorage.removeItem('pastEvents_backup'); loadPastEvents() }).catch(() => { if (firebaseDB) firebaseDB.ref('events').remove().then(() => { localStorage.removeItem('pastEvents_backup'); loadPastEvents() }) }) }
function copyEventLink(id) { const l = window.location.origin + window.location.pathname + '?event=' + id; navigator.clipboard.writeText(l).then(() => alert('✅ Link Copied:\n\n' + l)); }
...
async function editEventById(id) {
  try {
    ...
    if (data.logo) { currentLogoBase64 = data.logo; document.getElementById('logoPreview').src = data.logo; document.getElementById('logoPreview').classList.remove('hidden'); document.getElementById('logoSizeInfo').innerText = `✅ Logo Loaded - ${(data.logo.length / 1024).toFixed(1)}KB`; }
    if (data.banner) { currentBannerBase64 = data.banner; document.getElementById('bannerPreview').src = data.banner; document.getElementById('bannerPreview').classList.remove('hidden'); document.getElementById('bannerSizeInfo').innerText = `✅ Banner Loaded - ${(data.banner.length / 1024).toFixed(1)}KB`; }
    ...
    document.getElementById('cloudStatus').innerText = `✅ Edit Loaded! Event:${data.title}`;
    window.scrollTo({ top: 100, behavior: 'smooth' });
    alert(`✅ Edit Loaded:\n\nEvent: ${data.title}\nPlace: ${data.place}`);
  } catch (e) { alert('Edit Failed: ' + e.message); }
}

function editCurrentEvent() { if (!currentEventId || currentEventId === 'event_') { alert('Past Events-ൽ Edit Button അടിക്കൂ'); return; } editEventById(currentEventId.replace(/event_/g, '')); }
...
async function createOrUpdateLink(isNew) {
  ...
  document.getElementById('cloudStatus').innerText = '☁️ Saving via REST + SDK + Local...';
  document.getElementById('createLinkBtn').innerText = '⏳ Saving...';
  ...
  const certTitle = document.getElementById('certTitle')?.value || 'Certificate of Appreciation';
  const certSub = document.getElementById('certSub')?.value || 'Noor Al Madrasa - Meelad Quiz 2K26';
  const certSub2 = document.getElementById('certSub2')?.value || 'For Excellent Participation';
  ...
  if (saved) {
    document.getElementById('cloudStatus').innerText = isNew ? `✅ SUCCESS! Event:${title} ID:${cleanId}` : `✅ SAME LINK UPDATED! Event:${title}`;
    document.getElementById('createLinkBtn').innerText = '💚 CREATE NEW LINK';
    document.getElementById('saveCloudBtn').innerText = '💾 SAVE CLOUD SAME LINK';
    if (navigator.clipboard) navigator.clipboard.writeText(link).catch(() => { });
    loadPastEvents(); renderQuestions();
  } else { document.getElementById('cloudStatus').innerText = '❌ Failed to save to cloud!'; }
}

function copyLink() { if (!lastGeneratedLink) { const l = localStorage.getItem('lastGeneratedLink_v14'); if (l) lastGeneratedLink = l; else { alert('Create Link First!'); return; } } navigator.clipboard.writeText(lastGeneratedLink).then(() => alert('✅ Link Copied:\n\n' + lastGeneratedLink)); }
function shareWhatsApp() { if (!lastGeneratedLink) { const l = localStorage.getItem('lastGeneratedLink_v14'); if (l) lastGeneratedLink = l; else return; } window.open('https://wa.me/?text=' + encodeURIComponent('🌙 Noor Quiz:\n\n' + lastGeneratedLink), '_blank'); }

async function loadCompetitorEvent() {
  const params = new URLSearchParams(window.location.search);
  const eventId = params.get('event');
  if (!eventId) {
    document.getElementById('cloudLoadStatus').innerText = 'ℹ️ No Event ID - Default View';
    document.getElementById('compEventNameBelow').innerText = 'Noor Al Madrasa';
    document.getElementById('compPlaceShow').innerText = 'Meelad Quiz 2K26';
    document.getElementById('compDetailsBelow').innerText = 'Create Link in Admin';
    return;
  }
  document.getElementById('cloudLoadStatus').innerText = '☁️ Loading Event: ' + eventId;
  let data = null;
  ...
  if (!data) { document.getElementById('cloudLoadStatus').innerText = '❌ Event Not Found! ID:' + eventId; return; }
  ...
  document.getElementById('compEventNameBelow').innerText = data.title || 'Noor Al Madrasa';
  document.getElementById('compPlaceShow').innerText = data.place || '';
  document.getElementById('compDetailsBelow').innerText = `${data.category || 'Madrasa'} | ${totalMinutes} Min | ${Math.min(eventQuestionCount, questions.length)} Qs`;
  document.getElementById('eventNameDisplay').innerText = data.title || '';
  document.getElementById('eventCountDisplay').innerText = `📝 Total:${questions.length} Show:${Math.min(eventQuestionCount, questions.length)} Time:${totalMinutes}Min`;
  document.getElementById('eventRandomDisplay').innerText = `${randomMode ? '🔀 Random ON' : '📝 Same'} | ${showCorrectWrong ? '✅ Show Correct Green' : ''}`;
  document.getElementById('compSubTitle').innerText = data.category || 'Meelad Quiz 2K26';
  document.getElementById('cloudLoadStatus').innerText = `✅ Loaded! ${data.title}`;
...
function startQuiz() {
  ...
  if (phoneLock) { const used = JSON.parse(localStorage.getItem('usedPhones') || '[]'); const eid = new URLSearchParams(window.location.search).get('event') || currentEventId; if (used.includes(phone + '_' + eid)) { err.innerText = '❌ ഈ ഫോൺ ഉപയോഗിച്ചു!'; err.classList.remove('hidden'); return; } }
  const deviceLock = (document.getElementById('setDeviceLock')?.checked ?? eventDeviceLock);
  if (deviceLock) { const used = JSON.parse(localStorage.getItem('usedDevices') || '[]'); const eid = new URLSearchParams(window.location.search).get('event') || currentEventId; if (used.includes(eid)) { err.innerText = '❌ ഈ Device-ൽ ചെയ്തു!'; err.classList.remove('hidden'); return; } }
  err.classList.add('hidden');
  if (!questions.length) { const s = JSON.parse(localStorage.getItem('questions_v13') || '[]'); if (s.length) questions = s; else questions = [{ q: "നബി ജന്മദേശം?", opts: ["മക്ക", "മദീന", "ത്വാഇഫ്", "യമൻ"], ans: 0 }]; }
  ...
  qq.opts.forEach((o, i) => {
    ...
    b.onclick = () => {
      ...
      if (ok) {
        ...
        score++;
        fb.innerText = '✅ Excellent! ശരി!';
        fb.className = 'text-center font-black mt-4 text-sm p-4 rounded-xl bg-green-100 text-green-700 border-2 border-green-500';
      } else {
        b.style.background = '#dc2626'; b.style.color = 'white'; b.style.borderColor = '#dc2626';
        b.innerText = `❌ ${String.fromCharCode(65 + i)}. ${o} - തെറ്റ്!`;
        if (showCorrectWrong) {
          const cb = document.getElementById(`opt-${qq.ans}`);
          if (cb) {
            cb.style.background = '#16a34a'; cb.style.color = 'white'; cb.style.borderColor = '#16a34a'; cb.style.borderWidth = '3px';
            cb.innerText = `✅ ${String.fromCharCode(65 + qq.ans)}. ${qq.opts[qq.ans]} - ശരി!`;
          }
          fb.innerText = `❌ തെറ്റ്! ശരി: ${String.fromCharCode(65 + qq.ans)}. ${qq.opts[qq.ans]}`;
          fb.className = 'text-center font-black mt-4 text-sm p-4 rounded-xl bg-red-100 text-red-700 border-2 border-red-400';
        }
      }
      if (ok || showCorrectWrong) fb.classList.remove('hidden');
...
function loadResults() { const l = document.getElementById('resultsList'); if (!l) return; let r = JSON.parse(localStorage.getItem('results_v13') || '[]'); if (!r.length) { l.innerHTML = '<p class="text-center py-6 text-gray-500">No Results Yet</p>'; return; } r.sort((a, b) => { if (b.score !== a.score) return b.score - a.score; return a.time - b.time }); l.innerHTML = ''; r.forEach((x, i) => { const med = i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`; l.innerHTML += `<div class="flex justify-between py-2.5 border-b text-xs hover:bg-yellow-50 px-2 rounded"><span>${med} ${x.name} - ${x.score}/${x.total}</span><span class="font-bold">${x.timeStr}</span></div>` }) }
function downloadResults() { let r = JSON.parse(localStorage.getItem('results_v13') || '[]'); if (!r.length) { alert('No Results'); return; } let csv = 'Rank,Name,Phone,Place,Score,Total,Time\n'; r.forEach((x, i) => csv += `${i + 1},${x.name},${x.phone},${x.place || ''},${x.score},${x.total},${x.timeStr}\n`); const b = new Blob([csv], { type: 'text/csv' }), u = URL.createObjectURL(b), a = document.createElement('a'); a.href = u; a.download = 'Results.csv'; a.click(); }
function clearAllResults() { if (!confirm('Delete All Results?')) return; localStorage.removeItem('results_v13'); loadResults(); }
...
async function masterGoogleLogin() {
  ...
  try {
    if (!window.firebase || !firebase.auth) { alert('Firebase Auth SDK is not loaded.'); return; }
    const provider = new firebase.auth.GoogleAuthProvider();
    provider.setCustomParameters({ prompt: 'select_account' });
    const result = await firebase.auth().signInWithPopup(provider);
    const user = result.user;
    if (user && user.email && user.email.toLowerCase() === MASTER_ADMIN_EMAIL) {
      masterLoggedIn = true;
      sessionRole = 'master';
      document.getElementById('masterLoginBox').style.display = 'none';
      document.getElementById('masterPanel').style.display = 'block';
      status.innerText = '👑 Master Admin active — ' + user.email;
      document.getElementById('competitorView').classList.add('hidden');
      document.getElementById('adminView').classList.remove('hidden');
      ...
      alert('Access Denied: Only ' + MASTER_ADMIN_EMAIL + ' is authorized as Master Admin.');
    }
  } catch (err) {
    console.error(err);
...
async function masterDb(path, method = 'GET', body = null) {
  ...
  const url = `${REST_URL}/${path}.json` + (token ? `?auth=${token}` : '');
...
function v4ImportFile(ev) {
  ...
  else { alert('Please export the sheet as CSV.'); }
}
...
window.addEventListener('load', () => {
  ...
  if (admin && !document.getElementById('v4QuestionBtn')) {
    const b = document.createElement('button');
    b.id = 'v4QuestionBtn';
